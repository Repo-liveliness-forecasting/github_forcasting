...
accuracy(train.issues.naive.pred, valid.issues.ts)
...
accuracy(train.issues.snaive.pred, valid.issues.ts)
...
accuracy(train.issues.hw.pred, valid.issues.ts)
...
accuracy(ma.dd.pred[(n.train+1):(n.train+n.valid)], valid.issues.ts)
...
```{r}
train.issues.linear.regr.m <- tslm(train.issues.ts ~ trend + season, lambda = 0)
train.issues.linear.regr.m 
train.issues.linear.regr.pred <- forecast(train.issues.linear.regr.m , h=n.valid)

accuracy(train.issues.linear.regr.pred, valid.issues.ts)
hist(valid.issues.ts - train.issues.linear.regr.pred$mean)
plot(valid.issues.ts - train.issues.linear.regr.pred$mean)
plot(train.issues.linear.regr.pred)
lines(valid.issues.ts)
```
