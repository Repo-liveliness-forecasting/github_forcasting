...
plot.all.pred <- function(all.forecast) {
  plot(issues.ts, main="Prediction", xlim=c(30, 53.3), xlab = 'Week', ylab = 'Number of Issues')
...
plot.pred <- function(forecast.with.interval.ts) {
  plot(issues.ts, main="Prediction Interval", xlim=c(30, 53.3), xlab = 'Week', ylab = 'Number of Issues')
...
# retrun the vector of qunatile of 0.975, 0.95, 0.05, 0.025
...
# all.hw.forecast.future <- sapply(1:n.sample, function(i) return( forecast()))

ets.test.model <- ets(issues.ts, model = "ZAA")
ets.test.model.pred <- forecast(ets.test.model, h=n.valid)
quantile.of.residuals <- get.quantile.of.residuals(all.hw.forecast)
# the prediction intrerval of each time stamp
ets.test.model.pred.confidence.interval <- forecast.confidence(ets.test.model.pred$mean, quantile.of.residuals)
# convert the prediction interval into time series object
ets.test.model.pred.confidence.interval.ts <- ts(ets.test.model.pred.confidence.interval, start = c(53, 4), end = c(56, 3), frequency = 7)
forecast.object <- forecast.manual.interval(x.train=issues.ts, f.train=ets.test.model$fitted, f.pred=ets.test.model.pred$mean, f.lower=ets.test.model.pred.confidence.interval.ts[,1:2], f.upper= ets.test.model.pred.confidence.interval.ts[,3:4])
...
plot(forecast.object, xlim=c(30, 56))
